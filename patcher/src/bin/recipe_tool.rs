...
fn run() -> Result<()> {
    ...
    match arguments.as_slice() {
        [_, command, sm64_path, oot_path, mm_path, output_path, recipe_path] if command == "create" => {
            create(
                Path::new(sm64_path),
                Path::new(oot_path),
                Path::new(mm_path),
        ...
        [_, command, sm64_path, oot_path, mm_path, recipe_path] if command == "verify" => {
            verify(
                Path::new(sm64_path),
                Path::new(oot_path),
                Path::new(mm_path),
...
fn create(
    ...
    let oot_source = marios_mask_builder::oot::stone_talon_source(&oot)?;
    let output = fs::read(output_path)
        .with_context(|| format!("could not read {}", output_path.display()))?;
    ensure!(
        find_source(&output, &oot_source).is_some(),
        "release output does not contain the complete locally derived OoT Talon source"
    );
    ensure!(
        mm.len() + sm64.len() + oot_source.len() < u32::MAX as usize,
        "combined inputs exceed the recipe offset limit"
    );

    eprintln!(
        "Indexing {} input bytes with {} buckets…",
        mm.len() + sm64.len() + oot_source.len(),
        BUCKET_COUNT
    );
    ...
    let (_, stats) = recipe::apply(&encoded, &mm, &sm64, &oot, &oot_source)?;
    ensure!(
        stats.oot_bytes >= oot_source.len(),
        "output does not contain a complete OoT-derived Talon asset"
    );
    print!("{}", stats.report());
...
fn verify(sm64_path: &Path, oot_path: &Path, mm_path: &Path, recipe_path: &Path) -> Result<()> {
    let sm64 =
        fs::read(sm64_path).with_context(|| format!("could not read {}", sm64_path.display()))?;
    let mm = fs::read(mm_path).with_context(|| format!("could not read {}", mm_path.display()))?;
    let oot =
...
impl<'a> SourceIndex<'a> {
    fn new(mm: &'a [u8], sm64: &'a [u8], oot: &'a [u8]) -> Self {
        let total = mm.len() + sm64.len() + oot.len();
        let mut index = Self {
            mm,
            sm64,
            oot,
        ...
        if total >= MIN_MATCH {
            for position in 0..=total - MIN_MATCH {
                if (position < mm.len() && position + MIN_MATCH > mm.len())
                    || (position < mm.len() + sm64.len()
                        && position + MIN_MATCH > mm.len() + sm64.len())
                {
                    continue;
                }
    ...
    fn commands_for(&mut self, output: &[u8]) -> Vec<Command> {
        ...
        let protected_oot = find_source(output, self.oot);
        while position + MIN_MATCH <= output.len() {
            if protected_oot == Some(position) {
                if literal_start < position {
                    push_literal(&mut commands, &output[literal_start..position]);
                }
                commands.push(Command::CopyOot {
                    offset: 0,
                    length: self.oot.len() as u32,
                });
                for written in position..position + self.oot.len() {
                    self.insert_output(output, written);
                }
                position += self.oot.len();
    ...
    fn match_length(&self, source_position: usize, output: &[u8]) -> usize {
        ...
        } else if source_position < self.mm.len() + self.sm64.len() {
            &self.sm64[source_position - self.mm.len()..]
        } else {
            &self.oot[source_position - self.mm.len() - self.sm64.len()..]
        };
    ...
    fn source_slice(&self, position: usize, length: usize) -> &[u8] {
        ...
        } else if position < self.mm.len() + self.sm64.len() {
            let offset = position - self.mm.len();
            &self.sm64[offset..offset + length]
        } else {
            let offset = position - self.mm.len() - self.sm64.len();
            &self.oot[offset..offset + length]
        }
    }
    ...
    fn push_input_copy(&self, commands: &mut Vec<Command>, source_position: usize, length: usize) {
        let (kind, offset) = if source_position < self.mm.len() {
            (0, source_position)
        } else if source_position < self.mm.len() + self.sm64.len() {
            (1, source_position - self.mm.len())
        } else {
            (2, source_position - self.mm.len() - self.sm64.len())
        };
        ...
        match commands.last_mut() {
            ...
            }) if kind == 0 && *previous_offset + *previous_length == offset => {
                *previous_length += length;
            }
            Some(Command::CopySm64 {
                offset: previous_offset,
                length: previous_length,
            }) if kind == 1 && *previous_offset + *previous_length == offset => {
                *previous_length += length;
            }
            Some(Command::CopyOot {
                offset: previous_offset,
                length: previous_length,
            }) if kind == 2 && *previous_offset + *previous_length == offset => {
                *previous_length += length;
            }
            _ if kind == 0 => commands.push(Command::CopyMm { offset, length }),
            _ if kind == 1 => commands.push(Command::CopySm64 { offset, length }),
            _ => commands.push(Command::CopyOot { offset, length }),
        }
    }
}
...
fn find_source(output: &[u8], source: &[u8]) -> Option<usize> {
    if source.len() < MIN_MATCH || source.len() > output.len() {
        return None;
    }
    output
        .windows(MIN_MATCH)
        .enumerate()
        .find_map(|(position, prefix)| {
            (prefix == &source[..MIN_MATCH] && output[position..].starts_with(source))
                .then_some(position)
        })
}
